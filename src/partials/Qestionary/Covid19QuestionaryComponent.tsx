...
export interface Covid19QuestionaryComponentProps {

}

const Covid19QuestionaryComponent: React.FC<Covid19QuestionaryComponentProps> = () => {
    const { values, errors, touched, setFieldValue, setFieldTouched } = useFormikContext<Covid19QuestionaryValues>();
                            ...
                            <FastField name={`data.${index}.answer`} key={`data.${index}.answer`}>
                                {({ field, form, meta }: { field: string, form: string, meta: string }) => {
                                    return <QuestionYesNo
                                        ...
                                        // @ts-ignore
                                        question={strings[`covid19_${index.toString()}`]}
                                        showBullet
                                        value={value.answer}
                                        onChange={async (data) => {
                                ...
                                {({ field, form, meta }: { field: string, form: string, meta: string }) => {
                                    return (
                                        <div className={`${hasAnswer ? 'pl-4' : ''}`}>
                                            {/* @ts-ignore */}
                                            <label className="text-sm text-gray-500 dark:text-gray-300 mb-2 inline-block" htmlFor={`data.${index}.more_info`}>{label}</label>
                                            <TextArea
...
export default Covid19QuestionaryComponent;